...
function BlogPreview({ setPage, openArticle }) {
  return (
    <section className="bg-white py-24">
      <div className="max-w-7xl mx-auto px-5 md:px-8">
        <Reveal className="flex flex-wrap items-end justify-between gap-6">
        ...
        <div className="grid md:grid-cols-3 gap-6 mt-14">
          {BLOG_POSTS.slice(0, 3).map((p, i) => <BlogCard key={i} post={p} delay={i * 90} onOpen={openArticle} />)}
        </div>
      </div>
    </section>
  );
...
function BlogCard({ post, delay = 0, onOpen }) {
  return (
    <Reveal delay={delay}>
      <button
        onClick={() => onOpen(post)}
        className="h-full w-full text-left rounded-2xl border border-navy-900/8 p-7 hover:shadow-[0_20px_50px_-20px_rgba(10,31,61,0.15)] hover:-translate-y-1 transition-all duration-300 bg-white"
      >
...
function ArticleView({ post, onBack, setPage }) {
  useEffect(() => {
    window.scrollTo({ top: 0 });
  }, [post]);
  return (
    <>
      <section className="bg-navy-950 pt-36 pb-16 relative overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto px-5 md:px-8 relative">
          <button onClick={onBack} className="flex items-center gap-2 text-white/50 hover:text-emerald-400 text-sm mb-8 transition-colors">
            <ArrowRight size={14} className="rotate-180" /> Back to blog
          </button>
          <div className="flex items-center gap-3 text-xs text-emerald-400 font-medium mb-5">
            <span className="flex items-center gap-1"><Tag size={12} /> {post.category}</span>
            <span className="w-1 h-1 rounded-full bg-white/20" />
            <span className="flex items-center gap-1 text-white/40"><Clock size={12} /> {post.readTime}</span>
          </div>
          <h1 className="text-white text-3xl md:text-4xl font-medium leading-tight" style={{ fontFamily: "'Fraunces', serif" }}>{post.title}</h1>
        </div>
      </section>
      <article className="bg-white py-16">
        <div className="max-w-3xl mx-auto px-5 md:px-8">
          {post.body.map((para, i) => (
            <p key={i} className="text-slate-600 text-[15px] leading-relaxed mb-6">{para}</p>
          ))}
          <div className="mt-12 pt-10 border-t border-navy-900/8 rounded-2xl bg-slate-50 p-7">
            <h3 className="text-navy-900 font-semibold mb-2">Want content like this for your clinic?</h3>
            <p className="text-slate-500 text-sm mb-5">HearGrow Media writes SEO blogs and produces AI short videos exclusively for hearing aid clinics and audiology practices.</p>
            <Button onClick={() => setPage("contact")}>Get Started</Button>
          </div>
        </div>
      </article>
...
function HomePage({ setPage, openArticle }) {
  return (
    <>
      ...
      <TestimonialsSection />
      <BlogPreview setPage={setPage} openArticle={openArticle} />
      ...
      <section className="bg-white py-20">
        <div className="max-w-6xl mx-auto px-5 md:px-8 grid lg:grid-cols-[1fr_280px] gap-14">
          <div>
            ...
            <div className="grid sm:grid-cols-2 gap-6">
              {filtered.length ? filtered.map((p, i) => <BlogCard key={i} post={p} delay={i * 70} onOpen={openArticle} />) : (
...
const FORMSPREE_ENDPOINT = "https://formspree.io/f/mdaqeeje";
...
export default function App() {
  ...
  const pages = {
    home: <HomePage setPage={setPageAndScroll} openArticle={openArticle} />,
  ...
  return (
    <div className={dark ? "dark" : ""}>
      ...
      <div className="bg-white min-h-screen">
        ...
        <main>{selectedPost ? <ArticleView post={selectedPost} onBack={closeArticle} setPage={setPageAndScroll} /> : pages[page]}</main>
